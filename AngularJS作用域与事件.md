...
其实，在<code>$apply()</code>方法被调用后，degist循环会不止执行一次，当调用了监听函数后，这个degist循环会又会从头到来来检查在这个循环中是否有值改变。如果任何一个改变被发现，这个对应的监听器就会被触发，如何没有model值改变，就停止循环。否则，degist循环继续知道所有的model值被检测一遍，或者最大的循环次数到达十次。因为degist循环不会超过十次。<p>
/*记住一点degeist循环至少执行两次*/
